...
public class task_2 {
    public static void main(String[] args) {
        ...
        scanner.nextLine(); // считываем перевод на новую строку
        String[] book = new String[n]; // создаем массив для хранения строк книги
        for (int i = 0; i < n; i++) {
            book[i] = scanner.nextLine(); // считываем строки книги
        }
        int m = scanner.nextInt(); // количество аллергенов
        scanner.nextLine(); // считываем перевод на новую строку
        String[] allergens = new String[m]; // создаем массив для хранения аллергенов
        String[] substitutes = new String[m]; // создаем массив для хранения замен для аллергенов
        for (int i = 0; i < m; i++) {
            String[] allergenSubstitute = scanner.nextLine().split(" - "); // считываем строку с аллергеном и его заменой
            allergens[i] = allergenSubstitute[0];
            substitutes[i] = allergenSubstitute[1];
        }
        scanner.close(); // закрываем сканнер
        
        // Заменяем аллергены на заменители в каждой строке книги
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                String allergen = allergens[j];
                String substitute = substitutes[j];
                // Заменяем аллерген на заменитель в строке книги, сохраняя регистр заменителя
                book[i] = book[i].replaceAll("(?i)\\b" + allergen + "\\b", substitute);
            }
        }
        
        // Выводим строки книги с замененными аллергенами
